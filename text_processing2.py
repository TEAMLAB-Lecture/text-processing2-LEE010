...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    no_digit_regex = re.compile(r'\D')

    digit = no_digit_regex.sub('', input_string)
    digit_string = ' '.join(map(lambda c: digit_map[c], digit))
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    split_str = list(
        filter(lambda x: x, underscore_regex.split(underscore_str)))
    ...
    if len(split_str) < 1:
        ...
        camelcase_str = split_str[0].lower(
        ) + ''.join(map(lambda s: s.capitalize(), split_str[1:]))
